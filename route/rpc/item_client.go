...
func InitItemClient() (err error) {
	once_item.Do(func() {
		...
		clients[common_config.Get("item.service_name").(string)] = itemClientWrapper
	})
	return err
}

func itemClientWrapper(ctx context.Context, rpc_name string, body_any *any1.Any) (error, *any1.Any) {
	clientValue := reflect.ValueOf(ItemClient)
